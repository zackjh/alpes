...
class F3Set(BaseRGBModel):
    ...
    class Impl(nn.Module):
        ...
        def __init__(
            ...
            # contextual module
            if use_ctx:
                self._ctx = GRUPrediction(
                    num_classes + 1, num_classes + 1, d_model, num_layers=1
                )

        def forward(
            self, frame, coarse_label=None, fine_label=None, hand=None, max_seq_len=20
        ):
            ...
            seq_mask = torch.ones(
                (batch_size, max_seq_len), dtype=torch.bool, device=self._device
            )
    ...
    def epoch(
        ...
        with torch.no_grad() if optimizer is None else nullcontext():
            for batch_idx, batch in enumerate(tqdm(loader)):
                frame, src_key_padding_mask = loader.dataset.load_frame_gpu(
                    batch, self._device
                )
                ...
                with torch.cuda.amp.autocast():
                    ...
                    coarse_loss = F.cross_entropy(
                        coarse_pred.reshape(-1, 2), coarse_label.flatten(), **ce_kwargs
                    )
                    ...
                    fine_bce_loss = F.binary_cross_entropy_with_logits(
                        fine_pred, fine_label.float(), reduction="none"
                    )
    ...
    def predict(self, frame, hand, use_amp=True):
        ...
        with torch.no_grad():
            ...
            coarse_pred_cls = torch.argmax(
                non_maximum_suppression(coarse_pred, self._window), axis=2
            )
        ...
        for i in range(len(fine_pred)):
            for j in range(len(fine_pred[0])):
                if (
                    ...
                    and sum(
                        fine_label[
                            max(0, i - delta) : min(len(fine_pred), i + delta + 1), j
                        ]
                    )
                    == 1
                ):
