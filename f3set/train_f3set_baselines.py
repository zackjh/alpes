...
def get_args():
    ...
    parser.add_argument(
        "--window", type=int, default=5, help="Non maximun suppression window size"
    )
...
class F3Set(BaseRGBModel):
        ...
        def __init__(
            ...
            if temporal_arch == "gru":  # single layer GRU
                self._pred_fine = GRUPrediction(
                    self._feat_dim, num_classes, d_model, num_layers=1
                )
            elif temporal_arch == "deeper_gru":  # deeper GRU
                self._pred_fine = GRUPrediction(
                    self._feat_dim, num_classes, d_model, num_layers=3
                )
                ...
                self._pred_fine = GCNPrediction(
                    self._feat_dim, num_classes, hidden_dim=d_model
                )
            elif temporal_arch == "actionformer":  # Actionformer
                self._pred_fine = ActionFormerPrediction(
                    self._feat_dim, num_classes, d_model=d_model
                )
    ...
    def __init__(
        ...
        self._model = F3Set.Impl(
            num_classes, feature_arch, temporal_arch, clip_len, step=step
        )
    ...
    def predict(self, seq, use_amp=True):
        ...
        with torch.no_grad():
            ...
            pred = non_maximum_suppression(pred, self._window)
            pred_cls = torch.argmax(pred, axis=2)
            return pred_cls.cpu().numpy(), pred.cpu().numpy()


def evaluate(model, dataset, classes, dataset_name, delta=0):
    pred_dict = {}
    ...
    # evaluation metrices
    print("[LOG][train_f3set_baselines.py] Created `error_sequences_baseline.txt`")
    f = open("error_sequences_baseline.txt", "w")
    edit_scores = []
    f1_event = dict()
    ...
    for video, (scores, support) in sorted(pred_dict.items()):
        ...
        preds_element, labels_element = [], []
        for i in range(len(pred)):
            pred_element = np.zeros(len(per_label), int)
            label_element = np.zeros(len(per_label), int)
            for sub_label in classes_inv[pred[i]].split("_"):
                if sub_label in per_label:
                    pred_element[per_label[sub_label] - 1] = 1
            for sub_label in classes_inv[label[i]].split("_"):
                if sub_label in per_label:
                    label_element[per_label[sub_label] - 1] = 1
            preds_element.append(pred_element)
            labels_element.append(label_element)

        # element F1 scores
        labels_element = np.array(labels_element).astype(int)
        preds_element = np.array(preds_element).astype(int)
        ...
        for i in range(len(pred)):
            for j in range(len(per_label)):
                if (
                    ...
                    and sum(
                        labels_element[
                            max(0, i - delta) : min(len(pred), i + delta + 1), j
                        ]
                    )
                    == 1
                ):
        ...
        for i in range(len(pred)):
            if label[i] != 0:
                print_gts.append(classes_inv[label[i]])
            if pred[i] != 0:
                print_preds.append(classes_inv[pred[i]])
        ...
        for i in range(len(preds)):
            if (
                preds[i] > 0
                and preds[i]
                in labels[max(0, i - delta) : min(len(preds), i + delta + 1)]
            ):
            ...
            if (
                preds[i] > 0
                and sum(labels[max(0, i - delta) : min(len(preds), i + delta + 1)]) == 0
            ):
                if preds[i] not in f1_event:
            ...
            if (
                labels[i] > 0
                and labels[i]
                not in preds[max(0, i - delta) : min(len(preds), i + delta + 1)]
            ):
    ...
    print("[LOG][train_f3set_baselines.py] Mean F1 (Event):", np.mean(f1))

    precision = f1_element[:, 0] / (f1_element[:, 0] + f1_element[:, 1] + 1e-10)
    recall = f1_element[:, 0] / (f1_element[:, 0] + f1_element[:, 2] + 1e-10)
    f1 = 2 * precision * recall / (precision + recall + 1e-10)
    print("[LOG][train_f3set_baselines.py] Mean F1 (Element):", np.mean(f1))

    print(
        "[LOG][train_f3set_baselines.py] Edit Score:",
...
def get_datasets(args):
    ...
    print("[LOG][train_f3set_baselines.py] Dataset Size:", dataset_len)
...
def load_from_save(args, model, optimizer, scaler, lr_scheduler):
    ...
    print("[LOG][train_f3set_baselines.py] Loading from Epoch {}".format(epoch))
    model.load(
...
def get_lr_scheduler(args, optimizer, num_steps_per_epoch):
    ...
    print(
        "[LOG][train_f3set_baselines.py] Using Linear Warmup ({}) + Cosine Annealing LR ({})".format(
...
def main(args):
    ...
    classes, train_data, val_data, train_data_frames, val_data_frames = get_datasets(
        args
    )
    ...
    for epoch in range(epoch, num_epochs):
        ...
        print(
            "[LOG][train_f3set_baselines.py] Epoch [{}] - Training Loss: {:0.5f}".format(
                epoch, train_loss
            )
        )
        print(
            "[LOG][train_f3set_baselines.py] Epoch [{}] - Validation Loss: {:0.5f}".format(
                epoch, val_loss
            )
        )
        ...
        if args.criterion == "loss":
            if val_loss < best_criterion:
                ...
                best_epoch = epoch
                print("[LOG][train_f3set_baselines.py] New Best Epoch!")
        elif args.criterion == "edit":
            if epoch >= args.start_val_epoch:
                val_edit = evaluate(model, val_data_frames, classes, args.dataset)
                if args.criterion == "edit" and val_edit > best_criterion:
                    best_criterion = val_edit
                    best_epoch = epoch
                    print("[LOG][train_f3set_baselines.py] New Best Epoch!")
        else:
            print("[LOG][train_f3set_baselines.py] Unknown Criterion:", args.criterion)
    ...
    print("[LOG][train_f3set_baselines.py] Best Epoch: {}\n".format(best_epoch))
    ...
    if args.save_dir is not None:
        ...
        for split in eval_splits:
            ...
            if os.path.exists(split_path):
                ...
                evaluate(model, split_data, classes, args.dataset)
