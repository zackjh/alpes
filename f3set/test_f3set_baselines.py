...
def main(model_dir, frame_dir, split, dataset):
    ...
    model = F3Set(len(classes) + 1, config['feature_arch'], config['temporal_arch'], clip_len=config['clip_len'],
                  step=config['stride'], window=config['window'], multi_gpu=config['gpu_parallel'])
    ...
    split_data = ActionSeqVideoDataset(classes, split_path, frame_dir, config['clip_len'], 
                                       overlap_len=0, crop_dim=config['crop_dim'], 
