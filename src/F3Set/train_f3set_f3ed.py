...
def evaluate(model, dataset, classes, delta=0, window=5, device='cuda'):
    ...
    mean_f1_event = np.mean(f1)
    print('Mean F1 (event):', mean_f1_event)
    print()
    ...
    mean_f1_element = np.mean(f1)
    print('Mean F1 (element):', mean_f1_element)
    print()

    mean_edit_score = sum(edit_scores) / len(edit_scores)
    print('Edit score:', mean_edit_score)
    return mean_f1_event, mean_f1_element, mean_edit_score
    ...
    for epoch in range(epoch, num_epochs):
        ...
        if args.criterion == 'loss':
            ...
            if epoch >= args.start_val_epoch:
                _, _, val_edit = evaluate(model, val_data_frames, classes, window=args.window)
