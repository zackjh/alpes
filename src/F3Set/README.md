...
From the parent Alpes repository, install the source tree in editable mode before
running these commands:

```
python -m pip install -e . --no-deps
python -m pip install -r src/F3Set/requirements.txt
```
...
## Basic usage
To train all baseline models, use `python -m src.F3Set.train_f3set_baselines <dataset_name> <frame_dir> -s <save_dir> -m <model_arch> -t <head_arch>`.
...
To perform inference with an already trained model, use `python -m src.F3Set.test_f3set_baselines <model_dir> <frame_dir> -s <split>` or `python -m src.F3Set.test_f3set_f3ed <model_dir> <frame_dir> -s <split>`. This will output results for 3 evaluation metrics (event-wise mean F1 score, element-wise mean F1 score, and edit score).
